"""
Configuration and Data Preprocessing Script
Task 1: Data Preprocessing

This script contains configuration constants and cleans/preprocesses scraped reviews data.
...
try:
    ...
    # Load .env file from project root (parent of notebooks directory)
    env_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '.env')
    load_dotenv(env_path)
except ImportError:
    # python-dotenv not installed or .env file not needed
    pass
...
# 
# HOW TO FIND THE CORRECT APP IDs:
# 1. Go to https://play.google.com/store/apps
# 2. Search for each bank's mobile banking app (e.g., "Commercial Bank of Ethiopia mobile")
...
# 
# OR use the helper script: python notebooks/find_app_ids.py
# This will test common variations automatically.

# App IDs - can be overridden via .env file
APP_IDS = {
    'CBE': os.getenv('CBE_APP_ID', 'com.cbe.mobilebanking'),  # Can be set in .env file
    'BOA': os.getenv('BOA_APP_ID', 'com.bankofabyssinia.mobilebanking'),  # Can be set in .env file
    'Dashen': os.getenv('DASHEN_APP_ID', 'com.dashenbank.mobilebanking')  # Can be set in .env file
}
...
# Scraping configuration - can be overridden via .env file
SCRAPING_CONFIG = {
    'reviews_per_bank': int(os.getenv('REVIEWS_PER_BANK', '400')),
    'lang': os.getenv('SCRAPING_LANG', 'en'),
    'country': os.getenv('SCRAPING_COUNTRY', 'et'),
    'max_retries': int(os.getenv('MAX_RETRIES', '3'))
}

# Data file paths - can be overridden via .env file
DATA_PATHS = {
    'raw': os.getenv('DATA_RAW_DIR', 'data/raw'),
    'raw_reviews': os.getenv('DATA_RAW_REVIEWS', 'data/raw/reviews.csv'),
    'processed_reviews': os.getenv('DATA_PROCESSED_REVIEWS', 'data/processed/reviews_processed.csv')
}
